...
Based on each admission record, the admission location is treated as a discrete variable. The most admission location is emergency room admit, which corresponds to the most admission type.
    ...
    adm_loc2 <- admission %>%
      ...
      summarise(count = n()) %>%
      filter(count <= 600)
    adm_loc <- bind_rows(adm_loc1, adm_loc2) %>% as.data.frame
...
Based on each unique patient, we can get a sense of the language distribution. The language is treated as a discrete variable. Most of the patients speak English.
    ...
    language2 <- people %>%
      ...
      summarise(count = n()) %>%
      filter(count <= 460)
    language <- bind_rows(language1, language2) %>% as.data.frame
...
Based on each unique patient, we can get a sense of the religion distribution. The religion is treated as a discrete variable. Most of the patients are catholic.
    ...
    religion2 <- people %>%
      ...
      summarise(count = n()) %>%
      filter(count <= 460)
    religion <- bind_rows(religion1, religion2) %>% as.data.frame
...
Since the death analysis for each admission makes more sense than the death analysis for each person, the death analysis is based on each admission record. 

    ```{r}
    death <- admission %>%
      summarise(sum(is.na(DEATHTIME))
      ) %>% as.data.frame
    notdeath <- admission %>%
      summarise(sum(!is.na(DEATHTIME))
      ) %>% as.data.frame
    Death <- data.frame(count = c(death[[1]], notdeath[[1]]))
    label_value <- paste('(', round(Death$count / sum(Death$count) * 100,
                                    1), '%)', sep = '')
    label <- paste("DEATH", "NOTDEATH")
    label <- paste(label, label_value, sep = '')
    as_tibble(Death) %>%
      ggplot(mapping = aes(x = 'portion', fill = count)) +
      geom_bar(width = 1) +
      coord_polar("y") +
      labs(x = '', y = '', title = '') +
      theme(axis.text = element_blank()) +
