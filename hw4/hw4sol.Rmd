...
x_train <- raw_data[1:130, 1:5] %>% as.matrix() %>% print()
y_train <- raw_data[1:130, 6] %>% as.matrix() %>% print()
dim(x_train)
dim(y_train)

x_test <- raw_data[131:154, 1:5] %>% as.matrix() %>% print()
y_test <- raw_data[131:154, 6] %>% as.matrix() %>% print()
dim(x_test)
dim(y_test)
...
```{r}
# model2 <- glm(y_train ~., family = binomial(link = 'logit'),
#               data = x_train)
# summary(model2)
```
